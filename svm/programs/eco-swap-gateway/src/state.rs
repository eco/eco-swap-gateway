...
///
/// Seeded by `user_reward_token_account` — unique per (user, mint) pair
/// without needing a separate nonce. The snapshot is consumed and closed by
...
impl SwapSnapshot {
    ...
    pub fn pda(user_reward_token_account: &Pubkey) -> (Pubkey, u8) {
        Pubkey::find_program_address(
            &[SNAPSHOT_SEED, user_reward_token_account.as_ref()],
            &crate::ID,
        )
    }
}
