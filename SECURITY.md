...
If you believe you have found a security vulnerability in Eco Swap Gateway — anything
that could lead to loss or freezing of funds, theft, unauthorized state changes, forged
swap outputs, denial of service, or any break of the protocol's safety or liveness
...
1. Open the **[Security tab](https://github.com/eco/eco-swap-gateway/security)** of this repository.
...
deployed on-chain — including but not limited to the EVM `EcoSwapGateway` contract and
the Solana `eco_swap_gateway` Anchor program, along with any helper scripts or
interfaces that affect deployed behaviour.
